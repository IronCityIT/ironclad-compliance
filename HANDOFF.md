...
| **The printed report said a control was "partly evidenced (0%)".** Partial by corroboration while addressing no point of focus; the guidance now says so. Target dates also split across lines. Scoring of such controls is left for a decision: 46.5% as scored, 43.7% if they earned nothing (`PRODUCTIZE_NOTES.md` §16.52) | Printing and reading the client report in headless Chrome |
| **Configured, the dashboard still could not sign anyone in.** A leftover API placeholder read as "not configured"; the Auth0 SDK came from a host the CSP refuses; the session-renewal iframe had no `frame-src` (`PRODUCTIZE_NOTES.md` §16.51) | Headless Chrome with a fake configuration under the page's own policy |
| **The dashboard could never run under its own security policy.** Its CSP forbids inline scripts and it booted from one: headless Chrome rendered 0 of 7 capabilities. `ironclad serve` also dropped three of the four security headers Firebase Hosting sends (`PRODUCTIZE_NOTES.md` §16.50) | Loading the page in a real browser under its own headers |
| **An evidence file name reached the working spreadsheet as a live formula.** A file named `=HYPERLINK(...)` sat unaltered in `control-register.csv` and `evidence-index.csv`. Now written with a leading apostrophe; the audit trail is left exact for the hash check (`PRODUCTIZE_NOTES.md` §16.49) | Building a package from an evidence folder with formula-named files |
| **A plain copy made two-year-old evidence current, without a word.** Without a manifest, items are dated by file mtime; `cp -r` took readiness 35.2% → 46.5% and stale 5 → 0. Now disclosed as a report caveat (`PRODUCTIZE_NOTES.md` §16.48) | Assessing the same documents before and after a copy |
| **The auditor package promised checks an auditor could not make.** `audit-trail.csv` lacked three of the nine hashed fields and the digest rule was written nowhere; a package built without `--report` labelled a re-render naming the client by id "as issued" (`PRODUCTIZE_NOTES.md` §16.47) | Following the package README's claims with standard tools only |
| **On MariaDB, every open remediation item looked newly raised on every run.** The rebuilt previous document had no `created_at`, which carry-over reads; it also dropped `modules_run`, which `compare` reads (`PRODUCTIZE_NOTES.md` §16.45) | Planning a second run against each store's previous document: 27 of 27 dates kept from a volume, 0 of 27 from MariaDB |
